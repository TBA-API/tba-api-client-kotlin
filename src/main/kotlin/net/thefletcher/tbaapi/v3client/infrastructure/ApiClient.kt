package net.thefletcher.tbaapi.v3client.infrastructure
...
open class ApiClient(val baseUrl: String) {
    ...
    protected fun updateAuthParams(requestConfig: RequestConfig) {
        if (requestConfig.headers["X-TBA-Auth-Key"].isNullOrEmpty()) {
            if (apiKey["X-TBA-Auth-Key"] != null) {
                if (apiKeyPrefix["X-TBA-Auth-Key"] != null) {
                    requestConfig.headers["X-TBA-Auth-Key"] = apiKeyPrefix["X-TBA-Auth-Key"]!! + " " + apiKey["X-TBA-Auth-Key"]!!
                } else {
                    requestConfig.headers["X-TBA-Auth-Key"] = apiKey["X-TBA-Auth-Key"]!!
                }
            }
        }
    }
